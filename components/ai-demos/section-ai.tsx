...
import { Group, Kit } from "@/components/ai-demos/kit"
      ...
      <Kit id="ai-tool">
        <div className="w-[30rem]">
          <Tool defaultOpen>
            ...
            <ToolContent>
              {/* 문자열을 주면 ToolOutput 이 shiki 로 하이라이트한다 —
                * 서버와 클라이언트의 결과가 달라 하이드레이션이 어긋난다.
                * 노드로 넘겨 그대로 그리게 한다. */}
              <ToolOutput
                output={<div className="p-3">20건 중 17건 수집 성공</div>}
                errorText={undefined}
              />
            </ToolContent>
          </Tool>
      ...
      <Kit id="ai-artifact">
        <div className="w-[30rem]">
          <Artifact>
            ...
            <ArtifactContent>
              <p className="text-sm">8월 트렌드 요약 …</p>
            </ArtifactContent>
          </Artifact>
        </div>
      </Kit>
      ...
      <Kit id="ai-context">
        <Context usedTokens={48200} maxTokens={200000}>
          ...
          <ContextContent>
            <ContextContentHeader />
            <ContextContentBody>
              <p className="text-sm text-muted-foreground">이번 대화에서 쓴 토큰</p>
            </ContextContentBody>
          </ContextContent>
        </Context>
      </Kit>
