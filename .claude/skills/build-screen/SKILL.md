...
- `data/foundation.json` 전역 · `data/components.json` 레시피(참조만, 리터럴 금지)
- 고친 뒤 `npm run gen`
- 편집 화면 `/components/<id>` · 판정 화면 `/preview`
- 새 컴포넌트는 `components/_draft/` — `components/ui/` 에 추가 금지


- 색 `bg-primary` `text-muted-foreground` — `#hex` 금지
- 간격 `p-4` `gap-2` `p-(--pad-card)`
- 컨트롤 기준 `--h-control` `--pad-control` · 탭 `--h-tab` `--pad-tab`
- 파일럿 컴포넌트는 자기 이름을 갖는다 — `--button-md-height`, `--input-sm-padding-x`,
  `--card-md-padding-x`. 손으로 `h-9` 를 적지 않는다.
- 화면 하나에만 다른 값을 입히려면 전역을 고치지 말고 그 화면 루트에 인라인으로 얹는다.
...
승인 없이 `components/ui` 에 새 파일을 만들지 않는다 — 검사기가 잡는다.

만들기로 했다면 `components/_draft/` 에 넣는다. 두세 화면에서 반복해 필요해진 뒤에
`components/ui` 로 승격하고, 그때 8상태(기본·호버·포커스·누름·비활성·로딩·오류·성공)를
갖추고 `scripts/gen-components.mjs` 의 `DOC` 에 정의를 적은 뒤 `npm run gen` 을 돌린다.
