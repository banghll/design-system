...
| `/changes` | 컴포넌트별로 저장한 값을 모아 한 번에 내보내는 자리 |
| `/preview` | 판정 화면 — 목록 · 폼 · 대시보드. 밀도는 여기서만 판단된다 |
| `/components/:id` | 컴포넌트 상세 — 모든 변형·상태 미리보기 + 토큰 편집 (button · input · card) |
...
| 파일 | 무엇 |
| --- | --- |
| [`data/foundation.json`](data/foundation.json) | 전역. **값이 리터럴로 적히는 유일한 층** |
| [`data/components.json`](data/components.json) | 컴포넌트 레시피. 파운데이션을 **참조만** 한다 |

```json
"md": { "height": "control.height", "paddingX": "spacing.2.5", "fontSize": "text.sm" }
```

리터럴을 허용하지 않는 이유는 하나다 — 한 자리라도 px 로 적히면 밀도 기준을
바꿨을 때 그 자리만 안 따라오고, 그때부터 파운데이션은 아무것도 정하지 못한다.

둘 중 하나를 고치고 `npm run gen` 을 돌리면 `app/globals.css` 의 생성 구획,
`design-system.json`, 생성 페이지가 전부 따라온다.

**브라우저에서 고칠 수도 있다.** `/components/button` 오른쪽 패널에서 참조를 고르면
`/preview` 세 화면이 즉시 따라온다. 값은 두 칸으로 나뉜다.

| 칸 | 무엇 |
| --- | --- |
| 초안 | 지금 만지는 값. 화면에는 바로 반영되지만 아직 결정은 아니다 |
| 저장 | «이걸로 간다» 고 정한 값. 컴포넌트를 옮겨 다녀도 남는다 |

개별 화면에서는 **저장까지만** 한다. 버튼을 만지다 입력으로 넘어가 비교하고 다시
돌아오는 게 실제 작업 순서라, 화면마다 내보내면 파일이 여러 벌 생기고 어느 게
최신인지 알 수 없게 된다. 여러 컴포넌트를 다 만진 뒤 [`/changes`](app/changes/page.tsx)
에서 한 번에 내보내고, 그 파일을 `data/components.json` 에 덮은 뒤 `npm run gen` →
커밋한다. 그때부터가 정본이다.

지금 열려 있는 것은 **button · input · card** 셋, 속성은
**height · paddingX · radius · fontSize · gap** 다섯뿐이다. 나머지 58개는
파일럿을 검수한 뒤에 넘긴다.
